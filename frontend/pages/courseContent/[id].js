            ...
            {quizzes.map((quiz) => {
              ...
              return (
                <li key={quiz._id} className="flex justify-between items-center space-y-5">
                  ...
                  {studentGrade ? (
                    // If grade exists, display the grade
                    <span className="text-green-600 font-semibold">Your Grade: {studentGrade.grade}</span>
                  ) : (
                    // If no grade, show the Take Quiz button
                    <button
                      onClick={() => router.push(`/quiz/${quiz._id}`)} // Adjust the route accordingly
                      className="px-4 py-2 w-40 bg-black text-white rounded hover:bg-blue-700 text-center"
                    >
                      Take Quiz
                    </button>
                  )}
