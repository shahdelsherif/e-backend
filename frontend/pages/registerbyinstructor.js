...
export default function RegisterByInstructor() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (data.success) {
      ...
      // Redirect to the instructor dashboard after successful registration
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-blue-300">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-sm">
        <h2 className="text-3xl font-semibold text-center text-black mb-6">Register Student</h2>
        <form onSubmit={handleSubmit}>
          ...
          >
            {isSubmitting ? 'Registering...' : 'Register By Instructor'}
          </button>
        </form>
        <div className="mt-4 text-center">
          <p className="text-sm text-gray-600">
            Need to go back?{' '}
            <Link href="/instructorDashboard" className="text-pink-600 hover:underline">Go to Dashboard</Link>
          </p>
        </div>
      </div>
    </div>
